...
export interface DeclarationLeaf { property: string; value: CSSPrimitive; conditions: string[]; selectors: string[]; }

export function flattenStyle(style: StyleObject, conditions: string[] = [], selectors: string[] = []): DeclarationLeaf[] {
  const leaves: DeclarationLeaf[] = [];
  for (const [key, value] of Object.entries(style)) {
    if (value === null || value === undefined) continue;
    if (typeof value === 'object') {
      if (key.startsWith('@')) leaves.push(...flattenStyle(value, [...conditions, key], selectors));
      else leaves.push(...flattenStyle(value, conditions, [...selectors, key]));
      continue;
    }
...
export function renderAtomicRule(className: string, leaf: DeclarationLeaf): { cssText: string; conflictKey: string } {
  ...
  for (const condition of [...leaf.conditions].reverse()) cssText = `${condition}{${cssText}}`;
