...
export class StyleRegistry {
  ...
  compile(style: StyleObject, label = 'style'): string {
    ...
    for (const leaf of flattenStyle(style)) {
      ...
      const className = `${this.configuration.prefix}-${label.replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 18)}-${stableHash(signature)}`;
  ...
  getCSS(): string { return Array.from(this.rules.values(), (rule) => rule.cssText).join('\n'); }
  getRules(): readonly AtomicRule[] { return Array.from(this.rules.values()); }
  clear(): void { this.rules.clear(); this.metadata.clear(); this.styleElement?.remove(); this.styleElement = null; }
