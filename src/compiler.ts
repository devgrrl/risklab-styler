...
function assertSafeToken(value: CSSPrimitive, path: string): void {
...
function assertStyleObject(value: unknown, path: string): asserts value is StyleObject {
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (typeof child === 'string' || typeof child === 'number') {
      if (!SAFE_PROPERTY.test(key)) throw new TypeError(`${path}.${key} is not a valid property`);
...
export function validateStyleDocument(value: unknown): asserts value is StyleDocument {
  ...
  for (const [name, frames] of Object.entries(document.animations ?? {})) {
    if (!/^[a-zA-Z_][a-zA-Z0-9_-]*$/.test(name)) throw new TypeError(`animations.${name} has an invalid name`);
    ...
    for (const [step, style] of Object.entries(frames)) {
      if (!SAFE_ANIMATION_STEP.test(step)) throw new TypeError(`animations.${name}.${step} is invalid`);
